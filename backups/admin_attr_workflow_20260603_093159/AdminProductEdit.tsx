  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">Manage product-specific variation media. Product images are shown before selection; after a shopper selects a variation, that variation’s main image and Variation Image Gallery become the source of truth.</p>
      </CardHeader>
...
function AttributesTab({ productId }: { productId: number | null }) {
  ...
  const [pendingAssignments, setPendingAssignments] = useState<{ attributeId: number; activeValueIds: number[]; sortOrder: number; galleryControlling?: boolean }[]>([]);
  ...
  const addAttribute = (attributeId: number) => {
    setPendingAssignments(prev => [...prev, { attributeId, activeValueIds: [], sortOrder: prev.length, galleryControlling: false }]);
  };
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground capitalize">
                        {(attr?.displayType ?? 'button')} selector · {selectedIds.length} of {values.length} values selected for variant generation{pa.galleryControlling ? ' · controls gallery' : ''}
                      </p>
