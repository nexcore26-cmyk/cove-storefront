...
function AttributesTab({ productId }: { productId: number | null }) {
  ...
  const updateAttributeValueMutation = trpc.attributes.updateValue.useMutation({
    onSuccess: () => {
      toast.success('Attribute value swatch saved');
      utils.attributes.list.invalidate();
      utils.attributes.getProductAttributes.invalidate({ productId: productId! });
      utils.products.byId.invalidate({ id: productId! });
    },
  ...
  const setAttributeDisplayType = (attr: any, displayType: 'button' | 'image' | 'color') => {
    if (!attr?.id) return;
  ...
  const saveAttributeValueSwatch = (value: any, patch: { image?: string; swatch?: string }) => {
    if (!value?.id) return;
        ...
        {pendingAssignments.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            {pendingAssignments.map((pa) => {
              ...
              const displayType = (attr?.displayType === 'image' || attr?.displayType === 'color') ? attr.displayType : 'button';
                  ...
                  {!isCollapsed && (
                    <div className="space-y-3 px-3 py-3">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Select
                          value={displayType}
                          onValueChange={(value) => setAttributeDisplayType(attr, value as 'button' | 'image' | 'color')}
