...
export default function ProductDetail() {
  ...
  const selectedAttributeValueIds = useMemo(() => selectedNewValues.map((value: any) => Number(value.id)).filter(Number.isFinite), [selectedNewValues]);
  ...
  const selectedVariantAttributes = useMemo(() => {
    ...
    for (const pa of visibleNewAttrs) {
      ...
      if (!value) continue;
      labels[getAttributeVariantName(pa)] = getAttributeValueLabel(value);
    }
    if (selectedAttributeValueIds.length > 0) labels.__selectedAttributeValueIds = selectedAttributeValueIds;
    return labels;
  }, [visibleNewAttrs, attrSelections, selectedAttributeValueIds, locale]);
                ...
                <div key={pa.attributeId} className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {enabledValues.map((v: any) => {
                      ...
                      const optionBundleOutOfStock = Boolean(v.isBundle && stock && stock.onlineQty === 0);
                      ...
                      const isDisabled = valueOutOfStock || optionBundleOutOfStock || combinationUnavailable;
