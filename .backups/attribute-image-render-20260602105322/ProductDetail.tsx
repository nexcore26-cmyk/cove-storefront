...
export default function ProductDetail() {
  ...
  const variantImages = useMemo(() => {
    ...
    const gallery = parseGalleryImages(selectedVariant.galleryImages);
    const imgs: string[] = [];
  ...
  const attributeValueImages = useMemo(() => {
    for (const value of [...selectedNewValues].reverse()) {
      const imgs: string[] = [];
      if (value.image) imgs.push(String(value.image));
      imgs.push(...parseGalleryImages(value.galleryImages));
                      ...
                      if (displayType === 'image' && v.swatch) {
                        return (
                          <button key={v.id}
                            ...
                            <img src={v.swatch} alt={valueLabel} className="w-full h-full object-cover" />
                            {valueOutOfStock && (
