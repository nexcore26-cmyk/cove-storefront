...
export async function getProducts(opts: {
  ...
  // If warehouseId filter is requested, join with warehouse_stock to filter by availability.
  // A product is "available" in a warehouse when:
  //   - it has a stock row in that warehouse, AND
  //   - quantity > outOfStockThreshold (if threshold is enabled), OR quantity > 0 (if threshold disabled)
...
export async function transferStock(opts: {
  ...
  // P9: If transferring FROM main warehouse, validate ONLINE+POS total won't exceed MAIN stock
  ...
  if (fromWarehouse?.type === 'main' && (toWarehouse?.type === 'online' || toWarehouse?.type === 'pos')) {
    ...
    const mainConditions: any[] = [eq(warehouseStock.warehouseId, opts.fromWarehouseId), eq(warehouseStock.productId, opts.productId)];
    ...
    // Get all ONLINE + POS warehouses
    const distributionWarehouses = await db.select().from(warehouses)
      .where(sql`${warehouses.type} IN ('online', 'pos')`);
    const distWhIds = distributionWarehouses.map(w => w.id);
    // Sum all currently distributed stock
    let distributedTotal = 0;
    if (distWhIds.length > 0) {
      const distConditions: any[] = [sql`${warehouseStock.warehouseId} IN (${sql.join(distWhIds.map(id => sql`${id}`), sql`, `)})`, eq(warehouseStock.productId, opts.productId)];
      ...
      distributedTotal = distRows.reduce((sum, r) => sum + r.qty, 0);
    }
...
export async function deductStockForOrder(orderId: number): Promise<void> {
  ...
  const { orderItems: orderItemsTable, products: productsTable,
    warehouseStock, warehouses, bundleItems } = await import('../drizzle/schema');
  ...
  await db.transaction(async (tx) => {
    for (const item of items) {
      ...
      if (prod?.type === 'bundle' && item.variantId) {
        const components = await tx.select().from(bundleItems)
        ...
        // P7: Check if this item is on pre-order path
