...
function buildVariantSignature(attributesPayload: Record<string, string>): string {
  return Object.entries(attributesPayload)
...
async function syncProductVariantsFromAttributes(
  ...
  productId: number,
  assignments: Array<{ attributeId: number; activeValueIds: number[]; sortOrder: number; isImageMaster?: boolean }>
) {
  const normalizedAssignments = assignments
    .map((assignment, index) => ({
      ...
      isImageMaster: assignment.isImageMaster === true,
    }))
  ...
  for (const variant of existingVariants) {
    bySignature.set(buildVariantSignature((variant.attributes ?? {}) as Record<string, string>), variant);
  }
  ...
  for (const combo of combinations) {
    ...
    const masterOptionValues = combo
      .filter((entry) => normalizedAssignments.some((assignment) => assignment.attributeId === entry.attr.id && assignment.isImageMaster))
...
export const attributesRouter = router({
  ...
  create: adminProcedure
    .input(
      z.object({
        name: z.string().min(1),
        displayType: z.enum(["button", "color", "image"]).default("button"),
  ...
  update: adminProcedure
    .input(
      z.object({
        ...
        displayType: z.enum(["button", "color", "image"]).optional(),
        // P8: global = participates in variant generation; custom = display-only
  ...
  setProductAttributes: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const normalizedAssignments = input.assignments.map((a, index) => ({
        ...
        isImageMaster: a.isImageMaster === true,
      }));
      ...
      const requestedMasterIds = normalizedAssignments.filter((a) => a.isImageMaster).map((a) => a.attributeId);
      ...
      for (const val of vals) {
        if (val.isBundle) {
          const onlineQty = onlineWh
            ...
            : 0;
          const posQty = posWh
            ? await computeBundleStock(val.id, posWh.id)
            : 0;
          ...
          const posQty =
            stocks.find((s) => s.warehouseId === posWh?.id)?.quantity ?? 0;
          result.push({ valueId: val.id, onlineQty, posQty, isBundle: false });
        } else {
          // No stock management — treat as always available
          result.push({
            valueId: val.id,
            onlineQty: 999,
            posQty: 999,
