...
export default function ProductDetail() {
  ...
  const imageMasterAttribute = useMemo(
    () => (visibleNewAttrs as any[]).find((pa: any) => pa.isImageMaster === true) ?? null,
    [visibleNewAttrs]
  );

  const selectedImageMasterValue = useMemo(() => {
    if (!imageMasterAttribute) return null;
    const valueId = attrSelections[imageMasterAttribute.attributeId];
    if (!valueId) return null;
    return getActiveAttributeValues(imageMasterAttribute).find((v: any) => v.id === valueId) ?? null;
  }, [imageMasterAttribute, attrSelections]);
  ...
  const imageMasterVariantSelections = useMemo(() => {
    const mapped: Record<string, string> = {};
    if (!imageMasterAttribute || !selectedImageMasterValue) return mapped;
    const attr = imageMasterAttribute.attribute ?? {};
    const attrName = attr.name ?? attr.labelEn ?? attr.nameEn ?? imageMasterAttribute.name ?? imageMasterAttribute.labelEn;
    const option = selectedImageMasterValue.labelEn ?? selectedImageMasterValue.name ?? selectedImageMasterValue.label;
    if (attrName && option) mapped[String(attrName)] = String(option);
    return mapped;
  }, [imageMasterAttribute, selectedImageMasterValue]);
  ...
  const selectedImageVariant = useMemo(
    () => hasNewAttrs ? findMatchingVariant(liveVariants, imageMasterVariantSelections) : selectedVariant,
    [liveVariants, hasNewAttrs, imageMasterVariantSelections, selectedVariant]
  );
  ...
  const variantImages = useMemo(() => {
    if (!selectedImageVariant) return null;
    const gallery = parseGalleryImages(selectedImageVariant.galleryImages);
    const imgs: string[] = [];
    if (selectedImageVariant.image) imgs.push(selectedImageVariant.image);
    imgs.push(...gallery);
    return imgs.length > 0 ? imgs : null;
  }, [selectedImageVariant]);

  const attributeValueImages = useMemo(() => {
    if (!selectedImageMasterValue) return null;
    const imgs: string[] = [];
    const primaryImage = getAttributeValueImage(selectedImageMasterValue);
    if (primaryImage) imgs.push(primaryImage);
    imgs.push(...parseGalleryImages(selectedImageMasterValue.galleryImages));
    return imgs.length > 0 ? imgs : null;
  }, [selectedImageMasterValue]);

  const displayImages = variantImages || attributeValueImages || productImages;
                ...
                <div key={pa.attributeId} className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {values.filter((v: any) => v.isEnabled !== false).map((v: any) => {
                      ...
                      const attrName = String(attr?.name ?? attr?.labelEn ?? attr?.nameEn ?? `Attribute #${pa.attributeId}`);
                      const optionImage = displayType === 'image'
                        ? (getAttributeValueImage(v) || getVariantOptionImage(liveVariants, attrName, valueLabel))
                        : null;

                      if (displayType === 'image' && optionImage) {
                        return (
                          <button key={v.id}
                            onClick={() => !valueOutOfStock && toggleAttributeSelection(pa.attributeId, v.id)}
                            title={valueLabel}
