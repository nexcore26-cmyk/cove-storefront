...
function ProductCard({ product, locale, onWishlist }: { product: any; locale: string; onWishlist?: (id: number) => void }) {
  ...
  const arabicImages = locale === 'ar' ? parseImages(product.imagesAr) : [];
  const images = arabicImages.length > 0 ? arabicImages : parseImages(product.images);
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    addItem({ productId: product.id, name: cleanName(product.name), slug: product.slug, image: mainImage, price: salePrice || price, quantity: 1 });
      ...
      <div className="relative overflow-hidden mb-4" style={{ aspectRatio: '3/4', backgroundColor: '#F5F3EF' }}>
        ...
        {product.isNew && (
          <span className="absolute top-3 left-3 text-[10px] font-semibold tracking-widest uppercase px-2 py-1 z-10"
            style={{ backgroundColor: '#9D7D39', color: '#FAFAF8', fontFamily: 'Montserrat, sans-serif' }}>{sysLabel('new_badge', 'New')}</span>
        )}
        {salePrice && (
        ...
        <div className="absolute bottom-0 left-0 right-0 py-3 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          ...
          <button onClick={handleAddToCart} className="text-xs font-semibold tracking-widest uppercase"
            style={{ color: '#FAFAF8', fontFamily: 'Montserrat, sans-serif' }}>{sysLabel('add_to_cart', 'Add to Cart')}</button>
        </div>
      </div>
      <div>
        <Link href={`/product/${product.slug}`}>
          <h3 className="text-sm font-medium mb-2 hover:text-[#9D7D39] transition-colors duration-200"
            style={{ fontFamily: 'Montserrat, sans-serif', color: '#242424' }}>{cleanName(product.name)}</h3>
        </Link>
        {product.sku && (
...
export default function Shop() {
  ...
  const queryCategorySlug = useMemo(() => {
    const query = location.includes('?') ? location.split('?')[1] : '';
    return new URLSearchParams(query).get('category') || undefined;
  }, [location]);
  const effectiveCategorySlug = categorySlug || queryCategorySlug;
  ...
  // Only show online-channel categories in the storefront
  const { data: categoriesData } = trpc.categories.list.useQuery({ locale });
  const categories = useMemo(() => (categoriesData || [])
    .filter((c: any) => c.channel === 'online' || c.channel === 'both')
    .map((c: any) => ({
      ...c,
      // Strip internal suffixes from display names: " web", " Web", " store", " Store"
      name: cleanName(c.name),
    })), [categoriesData]);

  // Resolve category from URL slug
  useEffect(() => {
    if (effectiveCategorySlug && categories.length > 0) {
      // Exact match first, then legacy prefix/suffix fallbacks for old URLs
      const found =
        categories.find(c => c.slug === effectiveCategorySlug) ||
        categories.find(c => c.slug === `main-${effectiveCategorySlug}`) ||
        categories.find(c => c.slug === `brass-${effectiveCategorySlug}`) ||
        categories.find(c =>
          c.slug.replace(/^(main-|brass-)/, '').replace(/-(web|store|event)$/, '') === effectiveCategorySlug
        );
      setSelectedCategoryId(found?.id);
    } else if (!effectiveCategorySlug) {
      setSelectedCategoryId(undefined);
    }
  }, [effectiveCategorySlug, categories]);

  // Never filter by featured=true since products may not have isFeatured set
  // The 'Featured' sort option just shows newest order (same as default)
  const queryInput = {
  ...
  const { data: productsData, isLoading } = trpc.products.list.useQuery({ ...queryInput, locale });
            ...
            {isLoading ? (
              ...
              <div className="text-center py-32">
                ...
                <p className="text-xs" style={{ color: '#8A8A82', fontFamily: 'Montserrat, sans-serif' }}>
                  {debouncedSearch ? `No results for "${debouncedSearch}"` : 'Try adjusting your filters or browse all products.'}
                </p>
                {!debouncedSearch && (
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                  {sortedProducts.map(product => (
                    <ProductCard key={product.id} product={product} locale={locale} onWishlist={handleWishlist} />
                  ))}
                </div>
