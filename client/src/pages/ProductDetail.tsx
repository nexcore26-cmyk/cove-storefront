...
// Clean legacy WordPress/Woodmart description HTML before rendering rich content
...
function getVariantImages(variant: any | null | undefined): string[] | null {
  if (!variant) return null;
  const gallery: string[] = (() => {
    const g = variant.galleryImages;
    if (!g) return [];
...
function localizedAttributeName(attr: any, pa: any, locale: string): string {
  const arabicName = attr?.nameAr ?? attr?.labelAr ?? pa?.nameAr ?? pa?.labelAr;
  const englishName = attr?.name ?? attr?.labelEn ?? attr?.nameEn ?? pa?.name ?? pa?.labelEn;
  const preferred = locale === 'ar' ? arabicName : englishName;
  return String((preferred || englishName || arabicName || `Attribute #${pa?.attributeId ?? ''}`).trim?.() || preferred || englishName || arabicName || `Attribute #${pa?.attributeId ?? ''}`);
}

function localizedAttributeValueLabel(value: any, locale: string): string {
  const arabicLabel = value?.labelAr ?? value?.nameAr;
  const englishLabel = value?.labelEn ?? value?.name ?? value?.label;
  const preferred = locale === 'ar' ? arabicLabel : englishLabel;
  return String((preferred || englishLabel || arabicLabel || '').trim?.() || preferred || englishLabel || arabicLabel || '');
}
export default function ProductDetail() {
  ...
  console.log("Current availableStock state:", availableStock);
  ...
  // System labels (translatable UI strings)
  const { data: systemLabelsData = [] } = trpc.translations.getSystemLabels.useQuery();
  const sysLabel = (key: string, fallbackEn: string): string => {
    const row = (systemLabelsData as any[]).find((l: any) => l.key === key);
    if (!row) return fallbackEn;
    return locale === 'ar' && row.labelAr ? row.labelAr : row.labelEn;
  };
  ...
  const { data: fetchedAvailableStock } = trpc.products.getAvailableStock.useQuery({
    productId: product?.id ?? 0,
    variantId: null,
  }, {
    enabled: !!product?.id,
    onSuccess: (data) => {
      ...
      console.log("Fetched availableStock from backend:", data.stock);
  ...
  // Some products, such as Classic 225, use Color as the gallery controller. Keep the
  // main product gallery tied to that selected value even after other options are selected,
  // so changing Color always switches the main image back to the selected color.
  const galleryControllerSelection = useMemo(() => {
    for (const pa of (newAttrs as any[])) {
      const valueId = attrSelections[pa.attributeId];
      if (!valueId) continue;
      const attr = pa.attribute ?? {};
      ...
      const controlsGallery = attr.galleryControlling === true || normalizedName.includes('color') || normalizedName.includes('colour');
      if (!controlsGallery) continue;
      const value = getProductVisibleAttributeValues(pa).find((v: any) => v.id === valueId);
      const option = value?.labelEn ?? value?.name ?? value?.label;
      if (attrName && option) return { [String(attrName)]: String(option) } as Record<string, string>;
    }
  ...
  // Max quantity allowed when a bundle variation is selected
  const maxBundleQty = useMemo(() => {
    if (!allNewAttrsSelected) return Infinity;
    let min = Infinity;
    for (const v of selectedNewValues) {
      const s = stockMap[v.id];
      if (s && s.isBundle) {
        if (s.onlineQty <= 0) return 0;
        min = Math.min(min, s.onlineQty);
      }
    }
    return min;
  }, [allNewAttrsSelected, selectedNewValues, stockMap]);
  ...
  // Images: use selected variant gallery if available, else locale-specific product images
  const productImages = useMemo(() => {
    const parseImages = (rawImages: any): string[] => Array.isArray(rawImages)
      ? rawImages
      : (typeof rawImages === 'string' ? (() => { try { return JSON.parse(rawImages || '[]'); } catch { return []; } })() : []);
    const arabicImages = locale === 'ar' ? parseImages((product as any)?.imagesAr) : [];
    const defaultImages = parseImages((product as any)?.images);
    const imgs = arabicImages.length > 0 ? arabicImages : defaultImages;
    return imgs.length > 0 ? imgs : [PLACEHOLDER_IMG];
  }, [product, locale]);

  const variantImages = useMemo(() => getVariantImages(selectedVariant), [selectedVariant]);

  const galleryControllerVariant = useMemo(
    () => findMatchingVariant(liveVariants, galleryControllerSelection),
    [liveVariants, galleryControllerSelection]
  );
  const galleryControllerImages = useMemo(() => getVariantImages(galleryControllerVariant), [galleryControllerVariant]);

  const displayImages = galleryControllerImages || variantImages || productImages;
  ...
  const handleSelect = (attrName: string, option: string) => {
    setSelections(prev => {
      const next = { ...prev };
      if (next[attrName] === option) {
        delete next[attrName];
      } else {
        next[attrName] = option;
      }
      return next;
    });
    setCurrentImage(0);
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      ...
      variantName: (selectedVariant as any)?.name ?? undefined,
      shippingClassId: (selectedVariant as any)?.shippingClassId ?? (product as any).shippingClassId ?? null,
      kuwaitOnly: (selectedVariant as any)?.kuwaitOnly ?? (product as any).kuwaitOnly ?? false,
    });
    setAddedToCart(true);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20">
            ...
            <div className="relative overflow-hidden mb-4 bg-gray-50" style={{ aspectRatio: '4/5' }}>
              ...
              {strikePrice && (
                <span className="absolute top-4 left-4 text-[10px] font-semibold tracking-widest uppercase px-2 py-1"
                  style={{ backgroundColor: '#C0392B', color: '#FAFAF8', fontFamily: 'Montserrat, sans-serif' }}>{sysLabel('sale_badge', 'Sale')}</span>
              )}
            </div>
            {displayImages.length > 1 && (
          ...
          <div className="flex flex-col">
              ...
              return (
                <div key={pa.attributeId} className="mb-6">
                  <p className="text-xs font-semibold tracking-widest uppercase mb-3"
                    ...
                    style={{ fontFamily: 'Montserrat, sans-serif', color: '#242424', textAlign: locale === 'ar' ? 'right' : 'left' }}>
                    {localizedAttributeName(attr, pa, locale)}
                    {selectedValueId && (() => {
                      const sv = values.find((v: any) => v.id === selectedValueId);
                      const svLabel = sv ? localizedAttributeValueLabel(sv, locale) : '';
                      return sv && svLabel ? (
                        <span className={locale === 'ar' ? 'mr-2 font-normal normal-case text-[11px]' : 'ml-2 font-normal normal-case text-[11px]'} style={{ color: '#9D7D39' }}>
                          — {svLabel}
                        </span>
                      ) : null;
                    })()}
                  ...
                  <div className={displayType === 'button'
                    ? "inline-grid grid-cols-1 items-stretch gap-2"
                    ...
                    {values.filter((v: any) => v.isEnabled !== false).map((v: any) => {
                      ...
                      const showroomOnly = valueOutOfStock && stock && stock.posQty > 0 && !stock.isBundle;

                      const attrNameForVariantImage = String(attr?.name ?? attr?.labelEn ?? attr?.nameEn ?? pa.name ?? pa.labelEn ?? '');
                      const optionNameForVariantImage = String(v.labelEn ?? v.name ?? v.label ?? '');
                      const variantSwatchUrl = attrNameForVariantImage && optionNameForVariantImage
                        ? getOptionImage(liveVariants, attrNameForVariantImage, optionNameForVariantImage)
                        : null;
                      const imageSwatchUrl = variantSwatchUrl || getAttributeValueImage(v);
                      const displayValueLabel = localizedAttributeValueLabel(v, locale);
                      if (displayType === 'image' && imageSwatchUrl) {
                        return (
                          <button key={v.id}
                            onClick={() => !valueOutOfStock && handleAttributeValueSelect(pa.attributeId, v.id)}
                            ...
                            <img src={imageSwatchUrl} alt={displayValueLabel} className="w-full h-full object-cover" />
                            {valueOutOfStock && (
                      ...
                      return (
                        <div key={v.id} className="flex items-center gap-2">
                          <button
                            ...
                            disabled={valueOutOfStock && !showroomOnly}
                            dir={locale === 'ar' ? 'rtl' : 'ltr'}
                            className={`px-4 py-2 text-xs whitespace-nowrap border transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed ${locale === 'ar' ? 'text-right' : 'text-left'}`}
                            style={{
                              borderColor: '#9D7D39',
                              backgroundColor: '#9D7D39',
                              color: isSelected ? '#000000' : '#FAFAF8',
                              fontFamily: 'Montserrat, sans-serif',
                            }}>
                            {displayValueLabel}
                            {showroomOnly && ` — ${sysLabel('showroom_only', 'Showroom only')}`}
                          </button>
                          {valueOutOfStock && !showroomOnly && (
                            <span style={{ color: '#E53E3E', fontWeight: 600, fontSize: '0.7rem', whiteSpace: 'nowrap' }}>
                              {sysLabel('out_of_stock', 'Out of stock')}
                            </span>
                          )}
                        </div>
                      );
            ...
            {!hasNewAttrs && selectable.map(({ name, options }) => {
              ...
              const legacyHasImageOptions = options.some(option => getOptionImage(liveVariants, name, option));
              return (
                <div key={name} className="mb-6">
                  ...
                  <div className={legacyHasImageOptions
                    ? "flex flex-row flex-wrap items-start gap-2"
                    : "inline-grid grid-cols-1 items-stretch gap-2"}>
                    {options.map(option => {
                      ...
                      return (
                        <button key={option}
                          ...
                          {option}
                          {isOutOfStock && ` — ${sysLabel('out_of_stock', 'Out of stock')}`}
                        </button>
                      );
                    })}
                  </div>
            ...
            {selectedVariant?.htmlBlockEnabled && selectedVariant?.htmlBlockContent?.trim() && (
              <div
                className="html-content mb-3 text-sm leading-relaxed"
                dir={locale === 'ar' ? 'rtl' : 'ltr'}
                style={{ color: '#4A4A48', fontFamily: 'Montserrat, sans-serif', ...(locale === 'ar' ? { textAlign: 'right' } : {}) }}
            ...
            <div className="mb-6">
              <p className="text-xs font-semibold tracking-widest uppercase mb-3"
                style={{ fontFamily: 'Montserrat, sans-serif', color: '#242424' }}>
                {isUnitBased ? sysLabel('quantity', 'Quantity') : `${sysLabel('quantity', 'Quantity')} (${measurementType})`}
              </p>
              {isUnitBased ? (
                <div className="flex items-center border w-32" style={{ borderColor: '#D4C9B8' }}>
                  ...
                  <input
                    type="number"
                    value={quantity}
                    onChange={e => setQuantity(Math.max(1, Math.min(availableStock !== null ? availableStock : Infinity, parseInt(e.target.value) || 1)))}
                  ...
                  />
                  <button onClick={() => setQuantity(q => Math.min(availableStock !== null ? availableStock : Infinity, q + 1))}
                    disabled={quantity >= (availableStock !== null ? availableStock : Infinity)}
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center border" style={{ borderColor: '#D4C9B8' }}>
                    <input
                      ...
                      step="0.5"
                      value={qtyValue}
                      onChange={e => setQtyValue(Math.max(0.001, Math.min(availableStock !== null ? availableStock : Infinity, parseFloat(e.target.value) || 0.001)))}
                  ...
                  : hasNewAttrs
                    ? newSystemStockStatus === 'out-of-stock'
                      ...
                      : newSystemStockStatus === 'showroom-only'
                        ? sysLabel('visit_showroom', 'Visit Our Showroom')
                        : !allNewAttrsSelected
                          ? sysLabel('select_options', 'Select Options')
                          : sysLabel('add_to_cart', 'Add to Cart')
              ...
              <span className="text-xs" style={{ color: '#8A8A82', fontFamily: 'Montserrat, sans-serif' }}>
                {isBundle
                  ? (bundleStock === null
                    ...
                    : bundleStock > 0
                      ? `${sysLabel('in_stock', 'In stock')} (${bundleStock} sets available)`
                      : sysLabel('out_of_stock', 'Out of stock'))
                  : isOutOfStock && isPreOrderAvailable
                    ? sysLabel('pre_order', 'Pre-Order')
                    : (isOutOfStock ? sysLabel('out_of_stock', 'Out of stock') : variantStock > 0 ? `${sysLabel('in_stock', 'In stock')} (${variantStock} available)` : sysLabel('in_stock', 'In stock'))}
              </span>
            </div>
              ...
              <div className="flex gap-6 mb-4">
                {(['description', 'details', 'shipping'] as const).map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)}
                    ...
                    {tab === 'description' ? sysLabel('description', 'Description') : tab === 'details' ? sysLabel('details', 'Details') : sysLabel('shipping', 'Shipping')}
                  </button>
                ))}
              </div>
              <div className="text-sm leading-relaxed" style={{ color: '#4A4A48', fontFamily: 'Montserrat, sans-serif' }}>
                {activeTab === 'description' && (
                  <div
                    ...
                    dangerouslySetInnerHTML={{ __html: cleanDescription((product as any).description || 'No description available.') }}
                  />
                )}
                {activeTab === 'details' && (
                  <div>
