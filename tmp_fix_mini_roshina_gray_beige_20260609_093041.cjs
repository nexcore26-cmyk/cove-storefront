...
const stamp = '20260609_093041';
const backupPath = '/home/manus/cove-storefront/backups/mini_roshina_gray_beige_fix_backup_20260609_093041.json';
...
async function main(){
  ...
  try {
    ...
    if (duplicateIds.length) {
      ...
      const [del] = await db.execute('DELETE FROM product_variants WHERE productId=390207 AND id IN (' + ph + ')', duplicateIds);
      console.log('Deleted Beige duplicate variants: ' + del.affectedRows);
    } else {
    ...
    console.log('Updated Gray activeValueIds rows: ' + attrUpdate.affectedRows);
    ...
    async function insertIfMissing(base, spec){
      ...
      if (existing.length) { console.log('Gray variant already exists: ' + spec.name + ' -> ids ' + existing.map(r=>r.id).join(',')); return existing[0].id; }
      const attrs = {}; attrs[ATTR_NAME] = spec.name;
      const sql = 'INSERT INTO product_variants (productId,wcVariantId,sku,name,attributes,price,salePrice,cog,image,weight,isActive,galleryImages,preOrderEnabled,preOrderLimit,preOrderUsed,isDownloadable,isVirtual,manageStock,shippingClassId) VALUES (?,?,?,?,CAST(? AS JSON),?,?,?,?,?,?,CAST(? AS JSON),?,?,?,?,?,?,?)';
      ...
      console.log('Inserted Gray variant: ' + spec.name + ' -> id ' + res.insertId);
      return res.insertId;
    }

    await insertIfMissing(basicBase, { sku: '12554-BASIC-MUB', name: 'Basic set + Roshina Mubkhar', price: '55.000', cog: '19.200', image: basicBase.image });
    await insertIfMissing(fullBase, { sku: '12554-FULL-MUB', name: 'Full set + Roshina Mubkhar', price: '78.000', cog: '35.500', image: fullBase.image });
